...
elements.pause.addEventListener("click", () => perform("暂停并保存到 Flash", async () => { const config = collectConfig(); config.enabled = false; setConfig(config); logConfig("发送暂停配置", config); await connected().stageAndRun(config, Command.APPLY_SAVE); await refresh(); }));
