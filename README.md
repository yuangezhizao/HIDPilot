...
默认配置在 USB 枚举完成后立即执行瞬时相对 X `+100`、等待 `200 ms`、瞬时相对 X `-100`，之后以相邻两轮开始时间至少间隔 `55 s` 的节奏重复。这里的移动量是 HID 相对报告单位，实际屏幕像素受操作系统指针加速影响。
...
- 动作支持 `1–60000 ms` 延时、`-500–500` X/Y 总位移、`-127–127` 滚轮位移及 `0–60000 ms` 移动时长、`10–1000 ms` 鼠标点击和键盘组合键点击，最多 32 项；固件会把大位移自动拆成合法 HID 报告。
- 每轮动作从开始执行到全部输入释放期间，板载黄色 USER LED（GPIO25，低电平点亮）保持点亮；等待、暂停、挂起、拔出及配置替换时熄灭。
...
页面中的循环周期、延时、移动时长和按住时长统一以秒输入，支持最多 3 位小数，例如 `55`、`0.5` 和 `0.05`；页面会在传输前无损换算为协议使用的整数毫秒。

页面底部操作日志按时间记录连接、读取、试运行、应用、保存、暂停、恢复、重启及动作列表编辑的开始和结果；日志仅保存在当前页面内存中，刷新页面后清空，最多保留最近 200 条。
...
4. “暂停并保存到 Flash”会把启用标志关闭并直接保存，无需再点击“保存到 Flash”；“恢复默认”会直接恢复并保存出厂默认配置。
